...
# Filter output from annotated vcf file. 

# Here I filter for variants which are present in exonic regions and are non-synonymous. Furthermore, I compute 
# the difference between the allele frequency in the tumor and control cells and use it as parameter to select interesting variants
# This approach works better than just accepting the variants labelled as 'PASS' by Mutect particularly if T cells are used as 
# controls.
# Additionally if a variant does not fullfil this criteria, but affects an AML gene present in the myeloid Illumina panel
# is also added to the final table. This can be useful to identify pre-leukemic mutations for examples if T cells are used as 
# controls.
# I also checked for synonymous mutations which occur in highly expressed genes (using Anne's gene expression data) and have a high 
# difference in allele frequency between the tumor and the T cells. These if present could be used as markers (similar to 
...
aml_genes <- read_delim("data/reference_data/myeloid_panel_genes.txt", delim = "\n",
                        col_names = F) %>% pull(X1)


# get aml variants if present
if(length(which(aml_genes %in% variants$Gene.refGene)) > 0){
  ...
  aml_variants <- variants %>%  filter(Func.refGene %in% c("exonic", "splicing"),
                    ...
                    left_join(af) %>% left_join(coverage) %>% 
                    left_join(read_af) %>% 
...
# get highly expressed genes from Anne's patients
...
# select final variants based on the difference between AF in the tumor and Tcells or in the 'PASS' filter with sufficient coverage
final_variants <- variants %>% 
                  filter(Func.refGene %in% c("exonic", "splicing"),
                        ExonicFunc.refGene %in% c("nonsynonymous_SNV", "frameshift_insertion",
                                                  "frameshift_deletion", "stopgain")) %>% 
                  filter(FILTER %in% c("PASS", "germline", "normal_artifact", "germline;normal_artifact")) %>%
                    left_join(af) %>% left_join(coverage) %>% left_join(read_af) %>% 
                    mutate(af_diff = (af_tumor - af_tcells)/af_tcells) %>% 
                    filter((af_diff > 2 & af_tumor > 0.1 & depth_tcells > 10) | (FILTER == "PASS" & depth_tcells > 10)) %>% 
                    mutate(cancer_gene = ifelse(cosmic == ".", "no", "yes"),
                           cosmic_id = gsub(".+(COSV.+)\\\\x3bO.+", "\\1", cosmic)) %>% 
