...
setwd("~/cluster/project/AML/nuclear_mutations/")
...
variants <- read_csv(file.path(directory, "data/P19003_1/mutation_counts/amplified_variants.csv"))

# read count table
count_table <- readRDS(file.path(directory, "data/P19003_1/mutation_counts/count_table/P19003_1_count_table.rds"))
```
...
Here the proportion of reads aligning to the targeted genes is shown as well as to other genomic areas such as exons, non-coding regions of genes (genes), mitochondria and non-coding regions (other). Only reads from cell barcodes present in cell ranger output are included. The full list of barcodes can be found in `r snakemake@input$barcodes`.
...
For each of the targeted genomic sites the table shows the mean number of raw counts in Anne's samples together with the estimated distance of the mutation to the polyA.
...
if(sample == "P186"){
  ...
  sample_seurat <- "P186_high"
  
}else if(sample == "P19003_1"){
  
  sample_seurat <- "P19003-11"
  
}else{sample_seurat <- sample}

# if Seurat with high-quality cells exists only show those cells in downstream plots
if(file.exists(file.path(directory, "../gene_expression/data", sample_seurat, "seurat/seurat_projected.rds"))){
  
  barcodes <- colnames(readRDS(file.path(directory, "../gene_expression/data", sample_seurat, "seurat/seurat_projected.rds")))
  
  seurat <- T
  
}else{barcodes <- count_table$cell_barcode %>% unique()
      seurat <- F}

# cretate the table with aggreagated values based on gene and mutational status
status_table <- count_table %>% filter(cell_barcode %in% barcodes) %>%  
...
Here I show the mutant and reference cells in the projected UMAP. A cell is labelled as mutant if at least one mutant UMI is detected.

<br>

```{r, include=seurat, eval=seurat}

umap_table <- count_table %>% dplyr::select(cell_barcode, symbol, status) %>% 
                left_join(readRDS(file.path(directory, "../gene_expression/data", 
                                            sample_seurat, 
                                            "seurat/seurat_projected.rds"))@meta.data %>% rownames_to_column(var = "cell_barcode"))

# make UMAP with Variants
umap_variants <-  ggplot(umap_table,
                         aes(x = umapx, y = umapy, color = factor(status, levels = c("dropout", "reference", "mutant")))) + 
                          geom_point(data = umap_table %>% filter(status == "dropout"), size = 0.75) +
                          geom_point(data = umap_table %>% filter(status != "dropout"), size = 1) +
                          theme_classic() +
